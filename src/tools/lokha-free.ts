    ...
    handler: async (
      ...
      return {
        ...
        caller: {
          ...
          isAgent: Boolean(caller?.isAgent || caller?.role === "agent"),
        },
        privileges: {
          description: info.description,
          isOwner: Boolean(caller?.isOwner),
          isCurator: Boolean(caller?.isCurator),
          isAuthor: Boolean(caller?.isAuthor || caller?.isAgent || caller?.role === "agent"),
          freeDraftSubmissions: Boolean(caller?.isAuthor || caller?.isCurator || caller?.isOwner || caller?.isPaid || caller?.isAgent || caller?.role === "agent"),
